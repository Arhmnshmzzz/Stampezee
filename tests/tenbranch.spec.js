...
test("Login and create 10 branches", async ({ page, context }) => {
  ...
  console.log(
    "Login navigation:",
    loginResponse?.url() || "No navigation detected"
  );
  ...
  for (let i = 1; i <= 8; i++) {
    ...
    await page.waitForSelector('input[placeholder="Cherabinth Toronto"]', {
      timeout: 15000,
    });
    await page.waitForTimeout(1000);
    ...
    await page
      .getByText("Kawlar BazarDhaka, Bangladesh", { exact: false })
      .click();
    await page.waitForTimeout(1000);
    ...
    try {
      await expect(
        page.getByRole("cell", { name: branchName }).first()
      ).toBeVisible({ timeout: 30000 });
    } catch (err) {
      console.warn(
        `⚠️ Branch "${branchName}" not visible yet — reloading page and retrying...`
      );
