...
test('test', async ({ page }) => {
  ...
  await page.locator('.h-12.rounded-full.flex.items-center.transition-all.duration-200.hover\\:bg-gray-100.w-12.justify-center.bg-gradient-to-tl').click();
...
// ✅ Wait until Stripe card iframe (not Apple/Google Pay) is visible
  ...
  await cardIframe.getByRole('textbox', { name: 'Card number' }).fill('4242 4242 4242 4242');
  await cardIframe.getByRole('textbox', { name: 'Expiration date MM / YY' }).fill('04 / 44');
