...
test('Login and create stamp card', async ({ page, context }) => {
  ...
  await page.getByRole('textbox', { name: 'Enter Your Email Address' }).fill('cuqilikaso@mailinator.com');
  ...
  // Continue your flow
  await page.goto('https://stp2.rootdevs.xyz/en/retailer/my-stamp-card?page=1&limit=10', { waitUntil: 'networkidle' });
  await page.getByRole('button', { name: 'New Stamp Card' }).nth(1).click(); 
  await page.getByRole('textbox', { name: 'Enter Stamp Card Name' }).fill('asdasdasdasd'); 
  await page.getByRole('textbox', { name: 'Enter Stamp Card Title' }).fill('sdasdasdasds'); 
  ...
  await page.getByRole('textbox', { name: 'Add offer details' }).fill('asdadasdasdas'); 
